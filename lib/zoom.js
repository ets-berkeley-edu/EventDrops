...
export default (container, dimensions, scales, configuration, data, callback) => {
    const onZoom = (data,index,element) => {
            ...
            requestAnimationFrame(() =>{
                ...
                delimiters(container, {'x': scalingFunction}, configuration.dateFormat);
    ...
    // Bind zoom behavior to the chart wrapper rather than the parent SVG so that chart translation will be respected.    
    const chartWrapper = container.select('.chart-wrapper');
    chartWrapper.call(zoom);
